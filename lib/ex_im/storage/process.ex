    ...
    receive do
      ...
      {from, {:write, table, key, value}} ->
        ...
        |> case do
             ...
             values ->
               new_values =
                 Enum.reduce(values, [], fn {v, false}, acc -> [{v, true} | acc]
      ...
      {from, {:read, table, key}} ->
        reply =
          ...
          |> case do
               nil ->
                 []
               values ->
                 Enum.filter(values, fn {_, false} -> true ; _ -> false end)
             end
          send_reply(from, reply)
          loop()
        ...
        |> case do
             nil ->
               :noop
             values ->
               new_values = Enum.reduce(values, [], fn {v, false}, acc -> [{v, true} | acc] ; other, acc ->
               [other | acc] end)
